...
export const getSignGesturesForWord = (word: string): SignGesture[] => {
  return word.split('').map(letter => {
    const gesture = getSignGestureForLetter(letter);
    if (gesture) {
      return gesture;
    } else {
      // Create a fallback gesture with all required properties
      return {
        ...
        handPositions: [{
          ...
          fingers: [
            { x: 70, y: 40, rotation: 0, length: 20, width: 8, isBent: false },
            { x: 75, y: 30, rotation: 0, length: 25, width: 8, isBent: false },
            { x: 80, y: 30, rotation: 0, length: 25, width: 8, isBent: false },
            { x: 85, y: 30, rotation: 0, length: 25, width: 8, isBent: false },
            { x: 90, y: 50, rotation: -30, length: 20, width: 8, isBent: false },
          ],
          rotation: { x: 0, y: 0, z: 0 }
        }]
      };
